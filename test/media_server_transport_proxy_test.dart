...
void main() {
  ...
  test('uses CONNECT when an HTTP proxy carries an HTTPS target', () async {
    final received = Completer<({String method, Uri uri})>();
    final proxy = await HttpServer.bind(InternetAddress.loopbackIPv4, 0);
    addTearDown(() => proxy.close(force: true));
    proxy.listen((request) async {
      if (!received.isCompleted) {
        received.complete((method: request.method, uri: request.uri));
      }
    ...
    await expectLater(
      transport.send(
        http.Request('GET', Uri.parse('https://media.invalid/Items/1')),
        timeout: const Duration(seconds: 2),
      ),
      throwsA(anything),
    );

    final connect = await received.future.timeout(const Duration(seconds: 2));
    expect(connect.method, 'CONNECT');
    expect(connect.uri.toString(), contains('media.invalid'));
  });
