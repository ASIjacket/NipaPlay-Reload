...
void main() {
  ...
  testWidgets('uncompressed detail images use the unified bytes loader',
      (tester) async {
    ...
    final imageUri = Uri.parse(
      'http://emby.invalid/Items/detail-id/Images/Primary',
    );
    ...
    await tester.pumpWidget(
      ChangeNotifierProvider(
        ...
        child: MaterialApp(
          home: CachedNetworkImageWidget(
            imageUrl: imageUri.toString(),
            shouldCompress: false,
            imageBytesLoader: (uri) async {
    ...
    expect(requestedImages, [imageUri]);
    expect(tester.takeException(), isNull);
  });

  testWidgets('the same image URL reloads when its bytes loader changes',
      (tester) async {
    final imageBytes = Uint8List.fromList(base64Decode(
      'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk'
      '+A8AAQUBAScY42YAAAAASUVORK5CYII=',
    ...
    final calls = <String>[];
    const imageUrl = 'http://emby.invalid/Items/detail-id/Images/Primary';

    Widget build(Future<Uint8List> Function(Uri) loader) {
      return ChangeNotifierProvider(
        ...
        child: MaterialApp(
          home: CachedNetworkImageWidget(
            ...
            imageUrl: imageUrl,
            shouldCompress: false,
            imageBytesLoader: loader,
          ),
        ),
      );
    }

    await tester.pumpWidget(build((_) async {
      calls.add('first');
      return imageBytes;
    }));
    await tester.pump();
  ...
  testWidgets('low-resolution retry stays on the injected bytes loader',
      (tester) async {
    final imageBytes = Uint8List.fromList(base64Decode(
      'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk'
      '+A8AAQUBAScY42YAAAAASUVORK5CYII=',
    ...
    for (var attempt = 0;
        attempt < 10 && requestedImages.length < 2;
        attempt += 1) {
      await tester.pump(const Duration(milliseconds: 100));
    }
    await tester.pump();

    expect(requestedImages, [imageUri, imageUri]);
  });
