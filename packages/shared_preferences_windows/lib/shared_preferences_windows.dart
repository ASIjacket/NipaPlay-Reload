...
Future<Map<String, Object>> _readFromFile(
  ...
  if (localDataFile != null && await localDataFile.exists()) {
    final String stringMap = await localDataFile.readAsString();
...
Future<bool> _writePreferences(
  ...
  // Capture this call's state before another setter mutates the cached map.
  final Map<String, Object> snapshot = preferences.map((key, value) =>
      MapEntry(key, value is List<String> ? List<String>.of(value) : value));
  final Future<bool> write = _pendingWrite.then((_) async {
    try {
      final File? localDataFile = await _getLocalDataFile(
        fileName,
        fs: fs,
