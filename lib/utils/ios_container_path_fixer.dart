...
class iOSContainerPathFixer {
  ...
  static Future<String?> fixContainerPath(String originalPath) async {
    ...
    try {
      ...
      // 检查原路径是否包含不同的容器ID
      final originalContainerMatch =
          RegExp(r'/var/mobile/Containers/Data/Application/([^/]+)')
      ...
      debugPrint(
          'iOSContainerPathFixer: 路径修复 $originalContainerId -> $currentContainerId');
  ...
  /// 验证并修复文件路径
  ///
  /// 注意：这里刻意使用异步的 `exists()` 而不是 `existsSync()`。本方法在启动
  /// 阶段会对**每一条**观看历史各调用一次，而历史可能有几千条；同步 stat 在
  /// 慢速 eMMC（低端电视盒子）上每次都要数百微秒，串起来就是几秒的主线程阻塞。
  static Future<String?> validateAndFixFilePath(String originalPath) async {
    if (!Platform.isIOS) {
      return await File(originalPath).exists() ? originalPath : null;
    }
    ...
    // 尝试修复容器路径
    final fixedPath = await fixContainerPath(originalPath);
    if (fixedPath != null && await File(fixedPath).exists()) {
      return fixedPath;
    }
  ...
  /// 验证并修复目录路径
  static Future<String?> validateAndFixDirectoryPath(
      String originalPath) async {
    if (!Platform.isIOS) {
      return await Directory(originalPath).exists() ? originalPath : null;
    }
    ...
    // 尝试修复容器路径
    final fixedPath = await fixContainerPath(originalPath);
    if (fixedPath != null && await Directory(fixedPath).exists()) {
      return fixedPath;
    }
